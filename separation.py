...
def get_pathlengths_for_single_set(G,given_gene_set):
    ...
    f = open('gene_dis.txt', 'w')
    for gene1 in gene_set:
    	# modified to fit python 3.0
...
# =============================================================================
def calc_single_set_distance(G,given_gene_set, outfile):
        ...
        for geneB in gene_set:
            ...
            if geneA < geneB:
                if geneB in all_path_lenghts[geneA]:
                    lmin = all_path_lenghts[geneA][geneB]
                    if lmin < min_dis:
                        min_gene = geneB
                        min_dis = lmin
                ...
                if geneA in all_path_lenghts[geneB]:
                    lmin = all_path_lenghts[geneB][geneA]
                    if lmin < min_dis:
                        min_gene = geneB
                        min_dis = lmin
                    all_distances_A.append(all_path_lenghts[geneB][geneA])
        if len(all_distances_A) > 0:
            #print(geneA, min_gene, min_dis)
            if outfile:
                f.write(geneA+'\t'+min_gene+'\t'+str(min_dis)+'\n')
...
# =============================================================================
def calc_set_pair_distances(G,given_gene_set1,given_gene_set2, outfile):
...
if __name__ == '__main__':
    ...
    outfile = 'dis_a.txt'
    d_A = calc_single_set_distance(G,genes_A, outfile)
    outfile = 'dis_b.txt'
    d_B = calc_single_set_distance(G,genes_B, outfile)

    # distances BETWEEN the two gene sets:
    outfile = 'dis_ab.txt'
    d_AB = calc_set_pair_distances(G,genes_A,genes_B, outfile)
