...
# 
# ./localization.py -n interactome.tsv -g MS.txt -o output.txt
# 
#
# -----------------------------------------------------------------------
"""
...
# =================================================================================
def get_lcc_size(G,seed_nodes,is_write):
    """
    return the lcc size
    """
    ...
    if g.number_of_nodes() != 0:
        ...
        if is_write:
            subgraphs =G.subgraph(components[0]).edges()
            with open('core_gene_gene_interaction.txt', 'w') as f:
...
def get_random_comparison(G,gene_set,sims):
    ...
    for i in range(1,sims+1):
        ...
        # get rand lcc
        lcc = get_lcc_size(G,rand_seeds,False)
        l_list.append(lcc) 
        

    # get the actual value
    lcc_observed = get_lcc_size(G,gene_set,True)
...
if __name__ == '__main__':
    ...
    numbers = nx.degree(nx.subgraph(G,gene_set))
    gene_gene_interaction = nx.edges(nx.subgraph(G,gene_set))
   # print (">number of edges: %s" %(sedge))

    #output the genes localized in the Menche network
    with open('gene_set.txt', 'w') as f:
        for gene in gene_set:
            f.write(str(gene)+'\n')
    #output degree for each gene localized in the Menche network        
    with open('degree_gene_set.txt', 'w') as f:
        for gene in numbers:
            f.write(str(gene)+'\n')
    #output gene_gene_interaction genes localized in the Menche network        
    with open('gene_gene_interaction.txt', 'w') as f:
        for gene in gene_gene_interaction:
            f.write(str(gene)+'\n')            
    if len(gene_set_full) != len(gene_set):
        print ("> ignoring %s genes that are not in the network" %(len(gene_set_full - all_genes_in_network)))
    ...
    # get lcc size S
    lcc = get_lcc_size(G,gene_set,False)
    print ("\n> lcc size = %s" %(lcc))
    rg = nx.subgraph(G,gene_set)

    if rg.number_of_nodes() != 0:
        # get all components 
        components = list(nx.connected_components(rg))
    #output genes compose the LCC gene set
        with open('core_gene_set.txt', 'w') as f:
            for gene in components[0]:
            	f.write(str(gene)+'\n')
 
    # get mean shortest distance
    d_s = tools.calc_single_set_distance(G,gene_set, None)
